  ...
  return (
    <group key={stateString}>
      ...
      <mesh castShadow geometry={nodes.T_Shirt_male.geometry} material={materials.lambert1} material-roughness={1} dispose={null}>
        {/* You can use other 3D-specific properties here */}
        {snap.isFullTexture && (
          <Decal position={[0, 0, 0]} rotation={[0, 0, 0]} scale={1} map={fullTexture} />
        )}
        {snap.isLogoTexture && (
          <Decal position={[0, 0.04, 0.15]} rotation={[0, 0, 0]} scale={0.15} map={logoTexture} anisotrophy={16} depthTest={false} depthWrite={true} />
        )}
      </mesh>
    </group>
  );
